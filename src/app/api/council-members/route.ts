...
export async function POST(req: Request) {
  return handle(async () => {
    ...
    await logAudit({ organizationId: me.organizationId, userId: me.id, action: 'CREATE', entityType: 'CouncilMember', entityId: member.id, details: { councilId: b.councilId, memberUserId: b.userId } })
    return ok(member)
  })
}
